  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/users/register', {
        username: username, // Use username instead of ownername
        email,
        password,
        phone,
      });
      ...
      // Redirect to the loginOwner page with a success message
      setTimeout(() => {
        router.push('/user/loginuser'); // Adjust to your login page route
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-md p-8 w-full max-w-md">
          <div className="flex flex-row">
            <Image 
              ...
              priority
            />
            <h2 className="text-2xl font-bold text-center mb-6">Register as User</h2>
          </div>
          <input
            type="text"
          ...
          <button
            ...
            >
            {isSubmitting ? "Registering..." : "Register"} {/* Conditional button text */}
          </button>
        </form>
