...
const CurrentRentalDetails = ({ userId, token }) => {
    ...
    useEffect(() => {
        const fetchCurrentRentalDetails = async () => {
            try {
                ...
                console.log("Fetched rental details:", data); // Log the response data
    ...
    if (error) {
        return (
            <div className="text-center py-4 text-lg text-red-500">
                <strong>{error}</strong> {/* Display just the error message */}
            </div>
        );
    }
    ...
    // If rentalDetails is not an array, you can wrap it in an array to map over
