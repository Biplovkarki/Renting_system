...
export default function OwnerList() {
    ...
    return (
        <div className="p-6 bg-gray-100 min-h-screen">
            <h2 className="text-3xl font-bold text-gray-800 mb-6">users details</h2>
            {users.length === 0 ? (
                        ...
                        <tbody>
                            {users.map((user, index) => (
                                ...
                                >
                                    ...
                                    <td className="px-4 py-3">{user.user_address || "N/A"}</td>
                                    <td className="px-4 py-3 text-center">
                                        {user.user_image ? (
                                            <img
                                                src={`http://localhost:5000/uploads/user/${user.user_image}`}
                                                alt="user"
                                                className="h-16 w-16 rounded-full object-cover shadow-sm cursor-pointer"
                                                onClick={() =>
                                                    openModal(`http://localhost:5000/uploads/user/${user.user_image}`)
                                                }
                                            />
                                        ) : (
